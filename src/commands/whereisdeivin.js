  ...
  if (targetRole) {
      ...
      if (channel.type === 'voice' && channel.members) {
        channel.members.map((member) => {
          ...
          if (findRole) {
            ...
            let game = member.user.presence.activities.filter(
    ...
    if (findRole) {
      if (memberTarget.play) {
        message.reply(
          ` Nosso querido ${memberTarget.name} está no canal ${memberTarget.channel} jogando ${memberTarget.play}. Cumprimente-o!`
        );
        findRole = 0;
      } else {
        message.reply(
          ` Nosso querido ${memberTarget.name} está no canal ${memberTarget.channel}. Cumprimente-o!`
        );
        findRole = 0;
      }
    } else {
      message.reply(
        ` Infelizmente quem você está procurando não está entre nós D:`
      );
    }
  }
};
